...
function getRoutes(result, mask) {
  console.log(result);
  const routes = filter(result.map(route => {
    // 公共交通
    if (route.mode === undefined) {
      const steps = filter(route.steps, step => step.mode === 'TRANSIT');
      ...
      let duration;
      let fare;
      let lines = filter(step.lines, line => line.vehicle === 'BUS' || line.vehicle === 'SUBWAY');
  ...
  let maxScore = 0;
  const retRoutes =  routes.map(route => {
    route.score = direction(route.polyline, route.timespan, route.type, mask, route.fare, _s, _t, _m);
    if (maxScore <= route.score) maxScore = route.score;
    return route;
  }).sort((a, b) => a.score > b.score ? -1 : 1);
  return retRoutes.map(route => {
    route.score = route.score / maxScore;
